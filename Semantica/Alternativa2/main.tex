...
\AxiomC{$\pi_{type} \vdash_t \theta$}
\QuaternaryInfC{$\pi_{type} \vdash_t \T{array} \; r_1, \ldots, r_n \; \T{of} \; \theta$}
\end{prooftree}
...
\begin{prooftree}
\AxiomC{$init + size < limiteMaximo$}
\RightLabel{\quad cuando $size \in \NT{nat}$}
\UnaryInfC{$\pi_{type} \vdash_r init \; : \; size$}
\end{prooftree}
...
\AxiomC{$\pi \vdash_t \theta$}
\QuaternaryInfC{$\pi \vdash_t \T{array} \; r_1, \ldots, r_n \; \T{of} \; \theta$}
\end{prooftree}
...
\ER{Regla M para Tipos}{Rangos Constantes}
\begin{prooftree}
\AxiomC{$init + size < limiteMaximo$}
\RightLabel{\quad cuando $size \in \NT{nat}$}
\UnaryInfC{$\pi \vdash_r init \; : \; size$}
\end{prooftree}

\ER{Regla M para Tipos}{Rangos Variables}
\begin{prooftree}
\AxiomC{\empty}
\RightLabel{\quad cuando $size \in \NT{bsize}$}
\UnaryInfC{$\pi \vdash_r init \; : \; size$}
\end{prooftree}
\PR{además, se agrega la variable $size$ al contexto $\pi_{bound}$.}
...
\begin{prooftree}
\AxiomC{\empty}
\UnaryInfC{$tv_1, \ldots, tv_n : c_1, \ldots, c_m$}
\end{prooftree}
\PR{donde todas las variables $tv_i$ son distintas entre sí, y las clases $c_j$ también. Además, se satisface que $tv_i \in \pi_{typevar}$. Finalmente, se agregan todos los pares $(tv_i, c_j)$ al contexto del prototipo del método correspondiente.}
...
\ER{Regla B para Tipos}{Rangos Constantes}
\begin{prooftree}
\AxiomC{$init + size < limiteMaximo$}
\RightLabel{\quad cuando $size \in \NT{nat}$}
\UnaryInfC{$\pi \vdash_r init \; : \; size$}
...
\todo[color=green]{¿Esta bien sustituir también los tamaños variables? Suena muy restrictivo}
...
\begin{prooftree}
\AxiomC{$\pi \vdash_e v : \T{array} \; r_1, \ldots, r_n \; \T{of} \; \theta$}
\AxiomC{$\pi \vdash_e e_1 : \theta_1$}
\AxiomC{\ldots}
\AxiomC{$\pi \vdash_e e_n : \theta_n$}
\QuaternaryInfC{$\pi \vdash_e v[e_1, \ldots, e_n] : \theta$}
\end{prooftree}
\PR{donde se satisface que $\forall i \in \{ 1 \ldots n \}. \; indexT(r_i) = \theta_i$.}
...
\begin{prooftree}
\AxiomC{$\theta \sim \theta'$}
\UnaryInfC{$\T{array} \; r_1, \ldots, r_n \; \T{of} \; \theta \sim \T{array} \; r_1, \ldots, r_n \; \T{of} \; \theta'$}
\end{prooftree}
